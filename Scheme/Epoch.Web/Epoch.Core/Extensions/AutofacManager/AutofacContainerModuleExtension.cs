...
using Epoch.Core.CacheManager;
using Epoch.Core.Configuration;
using Epoch.Core.Const;
using Epoch.Core.Dapper;
using Epoch.Core.DBManager;
using Epoch.Core.EFDbContext;
using Epoch.Core.Enums;
using Epoch.Core.Extensions.AutofacManager;
//using Epoch.Core.KafkaManager.IService;
//using Epoch.Core.KafkaManager.Service;
using Epoch.Core.ManageUser;
using Epoch.Core.ObjectActionValidator;
using Epoch.Core.Services;

namespace Epoch.Core.Extensions
{
    public static class AutofacContainerModuleExtension
    {
        ...
        public static IServiceCollection AddModule(this IServiceCollection services, ContainerBuilder builder, IConfiguration configuration)
        {
            ...
            if (DBType.Name == DbCurrentType.MySql.ToString())
            {
                ...
                //services.AddDbContext<EpochContext>();
                //mysql8.x的版本使用Pomelo.EntityFrameworkCore.MySql 3.1会产生异常，需要在字符串连接上添加allowPublicKeyRetrieval=true
                services.AddDbContextPool<EpochContext>(optionsBuilder => { optionsBuilder.UseMySql(connectionString, new MySqlServerVersion(new Version(8, 0, 11))); }, 64);
            }
            else if (DBType.Name == DbCurrentType.PgSql.ToString())
            {
                services.AddDbContextPool<EpochContext>(optionsBuilder => { optionsBuilder.UseNpgsql(connectionString); }, 64);
            }
            else
            {
                services.AddDbContextPool<EpochContext>(optionsBuilder => { optionsBuilder.UseSqlServer(connectionString); }, 64);
            }
            //启用缓存
