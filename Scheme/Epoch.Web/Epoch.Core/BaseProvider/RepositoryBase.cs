...
using Epoch.Entity;
using Epoch.Entity.SystemModels;

namespace Epoch.Core.BaseProvider
{
    public abstract class RepositoryBase<TEntity> where TEntity : BaseEntity
    {
        ...
        public RepositoryBase(EpochContext dbContext)
        {
            this.DefaultDbContext = dbContext ?? throw new Exception("dbContext未实例化。");
        }

        private EpochContext DefaultDbContext { get; set; }
        private EpochContext EFContext
        {
            get
            {
        ...
        public virtual EpochContext DbContext
        {
